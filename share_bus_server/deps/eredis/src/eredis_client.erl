...
start_link(ConnectArgs)->
    Host =proplists:get_value(ip,ConnectArgs),
    Port = proplists:get_value(port,ConnectArgs),
    Database  = proplists:get_value(db,ConnectArgs,0),
    Password = proplists:get_value(password,ConnectArgs,""),
    ReconnectSleep = proplists:get_value(reconnect_interval,ConnectArgs,100),
    ConnectTimeout = proplists:get_value(connect_timeout,ConnectArgs,5000),
    gen_server:start_link(?MODULE, [Host, Port, Database, Password, ReconnectSleep, ConnectTimeout],[]).
